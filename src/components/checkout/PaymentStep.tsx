  ...
  return (
    <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-2xl p-6 md:p-8 shadow-sm">
      ...
      <div className="flex flex-col gap-4 mb-8">
        ...
        {/* Digital Payment Block */}
        <label className={`flex items-start gap-4 p-5 rounded-2xl border-2 cursor-pointer transition-all ${method === 'ONLINE' ? 'border-blue-600 bg-blue-50/50 dark:bg-blue-900/10 shadow-[0_0_0_1px_rgba(37,99,235,1)]' : 'border-gray-200 dark:border-gray-700 hover:border-blue-300 dark:hover:border-gray-600'}`}>
          <div className="pt-0.5">
            <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center ${method === 'ONLINE' ? 'border-blue-600' : 'border-gray-300 dark:border-gray-600'}`}>
               {method === 'ONLINE' && <div className="w-2.5 h-2.5 bg-blue-600 rounded-full" />}
            </div>
          </div>
          <div className="flex-1">
            <div className="font-bold text-gray-900 dark:text-white text-lg">Online Secure Payment</div>
            <p className="text-sm font-medium text-gray-600 dark:text-gray-400 mt-1">Pay instantly via bKash, Nagad, Rocket, or Visa/MasterCard.</p>
            
            {/* Visual Logos mock */}
            {method === 'ONLINE' && (
              <div className="mt-4 flex gap-2 opacity-80">
                <div className="w-10 h-6 bg-pink-500 rounded flex items-center justify-center text-[10px] text-white font-bold">bKash</div>
                <div className="w-10 h-6 bg-orange-500 rounded flex items-center justify-center text-[10px] text-white font-bold">Nagad</div>
                <div className="w-10 h-6 bg-indigo-600 rounded flex items-center justify-center text-[10px] text-white font-bold">Card</div>
              </div>
            )}
          </div>
        </label>
