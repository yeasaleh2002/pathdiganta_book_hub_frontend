      ...
      <div className="w-full flex items-center justify-center px-4 py-8 sm:p-12 lg:p-24 animate-in fade-in slide-in-from-bottom-8 duration-700">
        <div className="w-full max-w-md border border-gray-200 dark:border-gray-800 rounded-2xl p-5 sm:p-10 shadow-sm bg-white/50 dark:bg-gray-900/50 backdrop-blur-sm">
          ...
          <div className="mb-10 text-center">
            <h2 className="text-3xl sm:text-4xl font-black text-gray-900 dark:text-white tracking-tight">{t("createAccount")}</h2>
            <p className="mt-3 text-gray-500 dark:text-gray-400 font-medium">{t("registerSubtitle")}</p>
          </div>

          <form className="space-y-5" onSubmit={handleSubmit(onSubmit)}>
            
            <div className="space-y-1.5">
              <label className="block text-xs font-black uppercase tracking-widest text-gray-600 dark:text-gray-400">{t("fullNameLabel")}</label>
            ...
            <div className="space-y-1.5">
              <label className="block text-xs font-black uppercase tracking-widest text-gray-600 dark:text-gray-400">{t("securePasswordLabel")}</label>
              <div className="relative group">
                <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
            ...
            >
              {isSubmitting ? <><ButtonSpinner /> {t("registering")}</> : <>{t("createFreeAccount")} <ArrowRight size={18} strokeWidth={3} /></>}
            </button>
          </form>
