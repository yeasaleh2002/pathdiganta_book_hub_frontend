    ...
    <div className="w-full bg-white dark:bg-gray-950 pb-16">
      ...
      <div className="border-b border-gray-100 dark:border-gray-900 bg-gray-50/50 dark:bg-gray-950/50">
        <div className="max-w-7xl mx-auto px-4 py-3 text-xs text-gray-500 dark:text-gray-400 font-medium">
          {t("home")} <span className="mx-2">/</span> {t("books")} <span className="mx-2">/</span> {book.category?.name || "Category"} <span className="mx-2">/</span> <span className="text-blue-600 dark:text-blue-400 font-semibold">{book.title}</span>
        </div>
      </div>
      ...
      <div className="max-w-7xl mx-auto px-4 pt-10">
           ...
           <div className="flex-1 flex flex-col">
             ...
             <div className="mb-6 pb-6 border-b border-gray-100 dark:border-gray-800">
                ...
                <p className="text-lg text-gray-600 dark:text-gray-400 mt-3">
                  {t("by")} <Link href={`/books?author=${encodeURIComponent(book.author?.name || "")}`} className="font-bold text-blue-600 dark:text-blue-400 hover:underline">{book.author?.name}</Link>
                </p>
             </div>
             ...
             <div className="mb-8 p-6 bg-blue-50/30 dark:bg-gray-900/30 rounded-2xl border border-blue-50 dark:border-gray-800">
               ...
               {book.stock > 0 ? (
                 <span className="inline-flex items-center gap-1.5 text-sm font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/20 px-3 py-1.5 rounded-full border border-emerald-100 dark:border-emerald-800/50">
                   <CheckCircle2 size={16} /> {t("inStock")}
                 </span>
               ) : (
             ...
             <div className="grid grid-cols-2 gap-y-6 gap-x-4 p-6 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-800">
               <div className="flex flex-col gap-1.5">
                 <span className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider">{t("publisher")}</span>
                 <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 line-clamp-1">{book.publisher?.name || 'N/A'}</span>
               </div>
               <div className="flex flex-col gap-1.5">
                 <span className="text-xs text-gray-500 dark:text-gray-400 uppercase font-bold tracking-wider">{t("isbn")}</span>
         ...
         {relatedBooks.length > 0 && (
           <div className="pt-10 border-t border-gray-200 dark:border-gray-800">
             <BookShelf title={t("topRatedBooks")} viewAllLink={`/books?category=${book.category?.name}`} books={relatedBooks} />
           </div>
         )}
